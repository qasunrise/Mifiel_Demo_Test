...
function Mochawesome (runner, options) {
  ...
  runner.on('pass', function (test) {
    ...
    if (config.passedScreenshot) {
      var idleStart = (test.timer) ? test.timer._idleStart.toString() + '_' : '';
      saveScreenshotToFile(config.reportScreenshotDir + '/' + (idleStart + test.fullTitle().replace(/[^\w_\-]/g, '_').toLowerCase()).substr(0,config.screenshotMaxFileLenght) + '.png');
    }
  });
...
function cleanTest (test) {
  ...
  var err = test.err ? _.pick( test.err, ['name', 'message', 'expected', 'actual', 'stack', 'scr'] ) : test.err,
    scr = '<img src="./screenshots/' + (idleStart + test.fullTitle().replace(/[^\w_\-]/g, '_').toLowerCase()).substr(0,process.env['MOCHAWESOME_MAXFILELENGHT']) + '.png">';
